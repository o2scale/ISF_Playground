...
async function verifyCoachFeatures() {
    try {
        ...
        if (loginRes.data && loginRes.data.token) {
            ...
            try {
                const assignmentsRes = await axios.get(`${API_URL}/v2/lms/coach/assignments`, config);
                console.log(`✅ GET /assignments: ${assignmentsRes.status} (Found ${assignmentsRes.data.length} assignments)`);
            } catch (err) {
            ...
            try {
                const gradingRes = await axios.get(`${API_URL}/v2/lms/coach/grading/submissions`, config);
                console.log(`✅ GET /grading/submissions: ${gradingRes.status} (Found ${gradingRes.data.length} submissions)`);
            } catch (err) {
                console.log(`⚠️ GET /grading/submissions might be missing or empty: ${err.message}`);
                // Try alternate route if this fails, based on outlined routes
            }

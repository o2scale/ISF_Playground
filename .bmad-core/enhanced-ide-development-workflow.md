...
3. **E2E Test Scenario Validation (Manual Browser Testing)**
   ...
   - Test scenarios documented in docs/qa/e2e/{story-id}-{feature-name}.md
   - Coverage: Minimum ONE test case per Acceptance Criteria (typically 2-4 test cases per AC)
   - Test execution: QA Agent manually follows test scenarios in browser, observes console errors, takes screenshots
   - Test quality standards:
       * Clear preconditions and steps
       * Observable expected results
       * Error state coverage (network failures, validation errors, empty states)
       * Responsive behavior coverage (mobile/tablet/desktop)

...
const AttendanceComponent = () => {
  ...
  const updateMetrics = (studentList = students) => {
    ...
    const notMarked = studentList.filter(
      (s) =>
        !s.attendance ||
        s.attendance.length === 0 ||
        s.attendance[0].status === null
    ).length;
    ...
    <div className="attendance-management">
      ...
      <div className="balagruha-scroll-container">
        <div className="balagruha-list">
          {balagruhas.map((balagruha) => (
            <div
              key={balagruha._id}
              className={`balagruha-card ${
                selectedBalagruha === balagruha._id ? "active" : ""
              }`}
              onClick={() => {
      ...
      {selectedBalagruha ? (
            ...
            {students.length > 0 ? (
              ...
              <div className="no-students-message">
                <div className="no-data-message">
                  No students are assigned to this Balagruha.
                </div>
              </div>
            )}
          </div>
        ...
        <div className="no-selection">
          ...
          <div className="no-data-message">
            Please select a Balagruha to view attendance
          </div>
        </div>
      )}
    </div>

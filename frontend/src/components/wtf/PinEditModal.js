        ...
        return (
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Note: To replace the file, you would need to upload a new one through your media management system.
            </p>
          </div>
        );
      ...
      <DialogContent className="max-w-2xl">
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-medium mb-2">Pin Title *</label>
              <Input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Pin Caption</label>
              <Input
                type="text"
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="text-sm text-gray-600 space-y-1">
                <div><strong>Content Type:</strong> {pin.contentType}</div>
                <div><strong>Originally Pinned:</strong> {new Date(pin.pinnedDate).toLocaleDateString()}</div>
                <div><strong>Expires:</strong> {new Date(pin.expiresAt).toLocaleDateString()}</div>
                {pin.originalAuthor && <div><strong>Original Author:</strong> {pin.originalAuthor}</div>}
              </div>
            </div>

...
function BalagruhaDashboard() {
  ...
  const notifications = [
    {
      id: 1,
      title: "Tooth Check Time!",
      message: "Abhilasha has a tooth check on 25 Mar",
      time: "2 hours ago",
      read: false,
    },
    {
      id: 2,
      title: "Hooray! Task Done!",
      message: "Computer Fun task is all done!",
      time: "3 hours ago",
      read: false,
    },
    {
      ...
      message: "New Math Puzzle for Group A friends",
      time: "5 hours ago",
      read: true,
    },
    {
      id: 4,
      title: "Fix-it Alert!",
      message: "Big Fan will be fixed tomorrow",
      time: "1 day ago",
      read: true,
    },
    {
      ...
      message: "This page will get even better tonight!",
      time: "1 day ago",
      read: true,
    },
  ];
  ...
  const generateOverviewCards = () => {
    return [
      ...
      },
      {
        id: "machine-overview",
        title: t.computers,
        value: `${machines.filter((m) => m.status === "active").length || 0}/${
          machines.length || 0
        }`,
        icon: "🖥️",
        color: "#87CEFA",
      ...
      },
      {
        ...
        title: t.repairJobs,
        value:
          machines.filter((m) => m.status === "maintainence").length || "0",
        icon: "🔧",
        color: "#FFFFE0",
      },
      {
        ...
        rows: machines
          ...
          .map((machine) => [
            ...
            machine.reportedDate
              ? new Date(machine.reportedDate).toLocaleDateString()
              : "N/A",
          ]),
      },
      "subject-progress": {
  ...
  if (error) {
    return (
      <div className="error-container">
        ...
        <button className="retry-button" onClick={getBalagruhaList}>
          Retry
        </button>
      </div>
    );
          ...
          <div className="content-right">
            ...
            {selectedCard && (
              <div className="table-container">
                <h2>
                  {
                    overviewCards.find((card) => card.id === selectedCard)
                      ?.title
                  }{" "}
                  Details
                </h2>
                    ...
                    students.map((student) => (
                      <div key={student._id} className="child-item">
                        ...
                        <div className="medical-issues">
                          ...
                          student.medicalIssues.length > 0 ? (
                            student.medicalIssues.map((issue, index) => (
                              <span
                                key={index}
                                className="issue-tag"
                                style={{
                                  backgroundColor: issue.name?.includes("Tooth")
                                    ? "#FF9999"
                                    : issue.name?.includes("Skin")
                                    ? "#FFFF99"
                                    : issue.name?.includes("Blood")
                                    ? "#FFCC99"
                                    : issue.name?.includes("Hot")
                                    ? "#FF9999"
                                    : "#99FF99",
                                }}
                              >
                ...
                {students.length > 0 ? (
                  students.map((student) => (
                    <tr key={student._id}>
                      ...
                      <td>
                        {(student.attendance && student.attendance[0]?.time) ||
                          "N/A"}
                      </td>
                      <td>
                        {(student.attendance && student.attendance[0]?.notes) ||
                          "No notes"}
                      </td>
                    </tr>
                  ))
              ...
              <div className="medical-stat-card">
                ...
                <div className="stat-info">
                  <div className="stat-value">
                    {
                      students.filter(
                        (s) => s.medicalIssues && s.medicalIssues.length > 0
                      ).length
                    }
                  </div>
                ...
                <tbody>
                  {messages.length > 0 ? (
                    messages.map((msg, index) => (
                      <tr
                        key={index}
                        className={msg.read ? "" : "unread-message"}
                      >
                        <td>
                        ...
                        <td>
                          {users.find((u) => u._id === msg.recipientId)?.name ||
                            "Unknown"}
                        </td>
